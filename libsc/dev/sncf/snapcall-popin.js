		...
		if (window.jQuery) {
			...
			var snapcall = {
				...
				updateDOM: function(state) {
					...
					switch (state) {
						case 'callReset':
							$('.snapcall-btn').css({'background' : snapcall.button.background.unactive_bounce, 'background-size' : '70px 70px'});
							$('.snapcall-btn').show();
						...
						case 'callRequesting':							
							$('.snapcall-btn').css({'background' : snapcall.button.background.requesting, 'background-size' : '70px 70px'});
							break;
						
						case 'callActive':
							$('.snapcall-btn').css({'background' : snapcall.button.background.active, 'background-size' : '70px 70px'});
							break;
			...
			$.get('https://' + domain + '/libsc/dev/sncf/snapcall-popin.css', function(res) {
				...
				$('.snapcall-popin').append(`
					<video id="webcam" autoplay="autoplay" style="display:none;"></video>
		            
		            <div class="bar">
		                <div class="bar-ico">
		                    <div class="bar-in">
		                        <img src="https://` + domain + `/libsc/dev/sncf/img/sncf_logo.png" />
		                    </div>
		                </div>
		                <a tabindex="1">
		                    <div class="bar-txt">
		                ...
		                <div class="bar-cmd">
		                    <div class="bar-in">
		                        <a tabindex="2">
		                            <span class="fa fa-minus bar-cmd-reduce"></span>
		                        </a>
		                        &nbsp;
		                        <a tabindex="3">
		                            <span class="fa fa-expand bar-cmd-expand"></span>
		                        </a>
		                    </div>
		                </div>
		            </div>
		            <div class="box">
	                	...
	                	<div class="snapcall-dialog-invite">
		                    <div class="snapcall-dialog-size-big">En cliquant, vous serez mis en relation gratuitement avec un t&eacute;l&eacute;conseiller.</div>
		                    <div class="snapcall-dialog-size-medium">Le service est disponible du lundi au vendredi de 07h à 20h</div>
		                    <div class="snapcall-dialog-size-small">* hors jours f&eacute;ri&eacute;s et hors &eacute;v&egrave;nements exceptionnels</div>
	                	</div>

	                	<div class="snapcall-dialog-rating">
		                    <div class="snapcall-dialog-size-big">Votre avis nous int&eacute;resse</div>
		                    <div class="snapcall-dialog-size-medium">Contact TER vous remercie de votre appel.</div>
	                	</div>
	                	
	                	<div class="snapcall-dialog-connect">
		                    <div class="snapcall-dialog-size-big">Echec de la connection</div>
		                    <div class="snapcall-dialog-size-medium">Une erreur est survenue lors de la connection au service. Nous vous invitons a r&eacute;essayer dans quelques minutes.</div>
		                </div>

		                <div class="snapcall-dialog-browser">
							<div class="snapcall-dialog-size-medium">Pour accéder à cette fonctionnalité il est recommandé d'utiliser les navigateurs suivants</div>
							<div class="box-browser-icons">
		                ...
		                <div class="snapcall-dialog-timeout">
		                    <div class="snapcall-dialog-size-big">Temps d'attente expir&eacute;</div>
		                    <div class="snapcall-dialog-size-medium">Merci de v&eacute;rifier l'activation de votre micro et votre connexion Internet.</div>
		                </div>
		            </div>
				`);
		    		...
		    		if (!DetectRTC.isWebRTCSupported) {
						...
						snapcall.getCallerInfo(function() {
							snapcall.initLog(function() {
								...
								snapcall.caller.mouseclick = getTimestamp();

								var lanchCall = function() {
									if (snapcall.call.state == 0) {
										snapcall.call.state = 1;
										snapcall.button.bid_id = $('.snapcall-popin').attr('bid-id');
										snapcall.button.log_id = $('.snapcall-popin').attr('log-id');
								...
								$('.snapcall-popin .bar-txt').click(function() {
									$('.snapcall-popin .box').slideDown();
								});

								$('.snapcall-popin .bar-cmd-reduce').click(function() {
									$('.snapcall-popin .box').slideUp();
								});
								...
								$('.snapcall-btn').hover(
									function() {
										if (snapcall.call.state >= 2) {
											$('.snapcall-btn').css({'background' : snapcall.button.background.hangup, 'background-size' : '70px 70px'});
										}
									},
									function() {
								...
								setInterval(function() {
									if ((getTimestamp() - snapcall.caller.mouseclick > 15) && snapcall.caller.popinViewed == false) {
										snapcall.caller.popinViewed = true;
										$('.snapcall-popin .box').slideDown();
									}
										...
										switch (tabindex){
											...
											case 5:
												...
												snapcall.updateLog({
													action: 'rate',
													log_id: snapcall.button.log_id,
													rating: snapcall.call.rate
												});
												break;
										}
									} else if ((e.keyCode == 37 || e.keyCode == 39) && tabindex == 5){
										if (e.keyCode == 37 && snapcall.call.rate >= 1){
											snapcall.call.rate -= 1;

											$('.snapcall-rating-star').each(function() {
												if ($(this).attr('value') <= snapcall.call.rate) {
													$(this).css({'color':'#FDD017'});
												} else {
													$(this).css({'color':'black'});
												}
											})
										} else if (e.keyCode == 39 && snapcall.call.rate <= 4){
											snapcall.call.rate += 1;
